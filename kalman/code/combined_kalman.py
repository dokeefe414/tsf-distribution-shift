...
# -----------------------------
# 2. Helper Function: Standard KF Step
# -----------------------------
...
P_std = P_adapt = P_window = P_cp = 1.0

# initial values
x_std[0] = x_adapt[0] = x_window[0] = x_cp[0] = y[0]

# parameters
Q_std, R_std = 1e-3, 1.0

Q_adapt, R_adapt = 1e-3, 1.0
alpha = 0.1

Q_window, R_window = 1e-3, 1.0
window_size = 20

Q_cp, R_cp = 1e-3, 1.0
threshold = 150  # change-point threshold
...
for k in range(1, n):
    ...
    # ---- Standard KF ----
    x_std[k], P_std, _ = kalman_step(x_std[k-1], P_std, y[k], Q_std, R_std)

    # ---- Adaptive KF (innovation-based) ----
    innovation = y[k] - x_adapt[k-1]
    error_sq = min(innovation**2, 10000)

    R_adapt = (1 - alpha) * R_adapt + alpha * error_sq
    Q_adapt = (1 - alpha) * Q_adapt + alpha * error_sq * 0.5

    x_adapt[k], P_adapt, _ = kalman_step(x_adapt[k-1], P_adapt, y[k], Q_adapt, R_adapt)

    # ---- Sliding Window KF ----
    if k > window_size:
        window = y[k-window_size:k]
        R_window = np.var(window)

    x_window[k], P_window, _ = kalman_step(x_window[k-1], P_window, y[k], Q_window, R_window)

    # ---- Change-Point KF ----
    innovation_cp = y[k] - x_cp[k-1]

    if abs(innovation_cp) > threshold:
        P_cp = 100  # reset uncertainty

    x_cp[k], P_cp, _ = kalman_step(x_cp[k-1], P_cp, y[k], Q_cp, R_cp)
...
plt.title("Kalman Filter Variants on Distribution Shift Data")
plt.xlabel("Time")
plt.ylabel("Value")
plt.legend()
